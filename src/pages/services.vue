    ...
    <div class="relative bg-yellow-900">
      <div class="absolute inset-0">
        <g-image
          ...
          :src="`${$page.services.image}?nf_resize=smartcrop&w=1400&h=600`"
          :alt="$page.services.name.fr"
        />
        <g-image
          class="block sm:hidden w-full h-full object-cover"
          :src="`${$page.services.image}?nf_resize=smartcrop&w=700&h=500`"
          :alt="$page.services.name.fr"
        />
        <div
      ...
      <div class="relative max-w-7xl mx-auto pt-28 pb-24 px-4 sm:py-32 sm:px-6 lg:px-8">
        <h1 class="text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl">
          <span v-if="$context.locale == 'fr-fr'">
            {{ $page.services.name.fr }}
          </span>
          <span v-if="$context.locale == 'en-gb'">
            {{ $page.services.name.en }}
          </span>
          <span v-if="$context.locale == 'ar-dz'">
            {{ $page.services.name.ar }}
          </span>
        </h1>
      </div>
    </div>
        ...
        <div class="bg-white shadow rounded-lg w-full py-10 sm:py-16">
          <div class="relative px-4 sm:px-6 lg:px-8">
            <div
              ...
              v-html="$marked($page.services.description.fr)"
            />
            <div
              v-if="$context.locale == 'en-gb'"
              class="prose prose-yellow prose-lg text-gray-700 mx-auto"
              v-html="$marked($page.services.description.en)"
            />
            <div
              v-if="$context.locale == 'ar-dz'"
              class="prose prose-yellow prose-lg text-gray-700 mx-auto"
              v-html="$marked($page.services.description.ar)"
            />
          </div>
        </div>
      </div>
  ...
  metaInfo () {
    return {
      ...
      meta: [
        ...
        {
          property: 'og:title',
          content: this.$page.services.title,
          vmid: 'og:title'
        },
        {
...
<page-query>
query {
  services: singlePage (path: "/content/services") {
   title,
   name{fr,en,ar},
   image,
